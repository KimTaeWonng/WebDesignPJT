        ...
        <v-row v-if="type == 'groupDetail'">
          ...
          <v-dialog v-model="dialog" max-width="80%">
            <v-card>
              <div
                class="text-center"
                style="font-weight: bold; font-size: 4vw; align-items: center"
              >
                <v-col>
                  <span style="color: #ff0000" @click.stop="event1()"
                    >그룹 신고</span
                  >
                  ...
                  <v-dialog v-model="dialog2" max-width="80%">
                    <v-card>
                      <div style="font-weight: bold; align-items: center">
                        <v-col>
                          <p style="font-size: 5vw; margin-left: 3%">
                            신고 내용을 기재해주세요.
                          </p>
                          <v-textarea
                            filled
                ...
                <v-col>
                  ...
                  <v-dialog v-model="dialog3" max-width="80%">
                    <v-card>
                      <div style="font-weight: bold; align-items: center">
                        <v-col>
                          <div
                          ...
                          >
                            ...
                            <div v-for="(member, i) in this.members" :key="i">
                              ...
                              <p style="font-size: 4vw">
                                {{ member.userNick }}
                                <span v-if="i == 0" style="color: #289672"
                                  >운영진</span
                                >
                              </p>
                            </div>
                ...
                <v-col v-show="this.isGmember == true">
                  <span
                    v-if="this.manager == this.userInfo.user_id"
                    @click="delGroup"
                    >그룹 삭제</span
                  >
...
export default {
  ...
  methods: {
    ...
    async quitGroup() {
      ...
      await http
        .delete(`/group/${this.$route.params.groupno}/${this.userInfo.user_id}`)
        .then((res) => {
          console.log(res);
          console.log("그룹 탈퇴");
