<template>
  <v-container>
    <v-row class="text-center">
      <v-col cols="12">
        <v-card
          style="text-align: left; font-size: 3vw"
          @click.stop="(dialog = true), checkmember()"
        >
          <v-dialog v-model="dialog" max-width="80%">
            <v-card>
              ...
              ></v-text-field> -->
                <div>
                  <v-icon>person_outline</v-icon> {{ this.membersid.length }} /
                  {{ this.meeting.meetingPeople }}명
                </div>
              </div>
              <br />
              <div class="text-center">
                <v-btn
                  ...
                  dark
                  width="35%"
                  @click.stop="(dialog = false), join()"
                >
                  가입
                </v-btn>
                <v-btn
                  v-else-if="this.manager == this.userInfo.user_id"
                  color="rgb(40,150,114)"
                  dark
                  width="35%"
                  @click.stop="del(), (dialog = false)"
                >
                  삭제
                </v-btn>
                <v-btn
          ...
          <div style="margin-left: 3%">
            <div>
              ...
              <v-row>
                <v-col cols="8">
                  <p style="font-weight: bold; float: left; font-size: 8vw">
              ...
              <div>
                ...
                <v-icon style="font-size: 3.5vw"> mdi-map </v-icon>
                <!-- 사용자 지정 장소로 대체 -->
                {{ meeting.meetingUrl }}
              </div>
            </div>
          ...
          <div
            ...
            style="font-size: 3.5vw; margin-left: 3%"
          >
            <!-- 사용자 지정 모임비로 대체 -->
            <p>₩ 모임비 {{ meeting.meetingMoney }}원</p>
            <!-- 현재인원/사용자 지정 참여인원 수로 대체 -->
            <p
              v-if="this.membersid.length < this.meeting.meetingPeople"
              style="margin-left: 5%"
            >
              참여가능 {{ this.membersid.length }} /
              {{ this.meeting.meetingPeople }}
            </p>
            <p v-else style="margin-left: 5%">
              참여불가 {{ this.membersid.length }} /
              {{ this.meeting.meetingPeople }}
            </p>
          </div>
        </v-card>
...
export default {
  ...
  methods: {
    async getmembers() {
      await http
        .get(`/group/meetings/members/${this.meeting.meetingId}`)
        .then((res) => {
          // console.log(res)
    ...
    async join() {
      ...
      await http
        .post(
          `/group/meetings/members/${this.meeting.meetingId}/${this.userInfo.user_id}`
        )
        .then((res) => {
          console.log(res);
          // console.log('정모 참가')
    ...
    async del() {
      await http.delete(
        `/group/${this.group.group_id}/meetings/{mid}?mid=${this.meeting.meetingId}`
      );
      this.$router.go();
    },
