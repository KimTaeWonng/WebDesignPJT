      ...
      <v-col cols="12">
        <v-row>
          ...
          <v-img
            ...
            contain
          />
          <v-img v-else src="@/assets/groupinfo.png" class="mt-3" contain />
        </v-row>
        <br />
        <br />
        ...
        <div style="text-align: left; vertical-align: middle">
          ...
          >
            ...
            <v-btn
              v-if="this.isGmember == false"
              @click="join"
              color="rgb(40,150,114)"
              dark
              width="20%"
              height="7vw"
              style="margin-left: 5%; font-size: 3vw"
            >
              가입
            </v-btn>
            <v-btn
              v-else-if="this.userInfo.user_id == this.manager"
              color="rgb(40,150,114)"
              dark
              width="20%"
              height="7vw"
...
export default {
  ...
  async created() {
    ...
    const members = await http.get(
      `/group/members/${this.$route.params.groupno}`
    );
  ...
  methods: {
    ...
    async join() {
      ...
      await http
        .post(`/group/${this.$route.params.groupno}/${this.userInfo.user_id}`)
        .then((res) => {
          console.log(res);
          // console.log('그룹 참가')
          this.isGmember = true;
    ...
    async quit() {
      ...
      const response = await http.delete(
        `/group/${this.$route.params.groupno}/${this.userInfo.user_id}`
      );
      if (response.data == 1) {
        // console.log('그룹 탈퇴')
    ...
    async registMeeting() {
      ...
      const response = await http.post(
        `/group/${this.$route.params.groupno}/meetings`,
        this.meeting
      );
      if (response.data == 1) {
      ...
      const getmeeting = await http.get(
        `/group/${this.$route.params.groupno}/meetings`
      );
      ...
      await http.post(
        `/group/meetings/members/${mid}/${this.userInfo.user_id}`
      );
    },
  },
};
